import { AreaChart } from './Charts'
import { Customers } from './Customers'
import { QuickActions } from './QuickActions'

import { useOverview } from '../../../context/overview'

export default function Overview () {
  const { state } = useOverview()
  const { customers, revenue, totalOrders, deliveredOrders, refund } = state
  return (
    <section className='h-full w-full pt-5 p-4 sm:p-6 md:p-10 overflow-y-auto'>
      <section>
        <h2 className='text-base sm:text-lg md:text-xl font-semibold mb-3 sm:mb-5'>
          Analytics
        </h2>
        <ul className='w-full grid max-sm:mb-15 grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-10 sm:gap-3 md:gap-5 lg:gap-7 lg:pr-20'>
          <li className=''>
            <header className='flex rounded-lg items-center bg-[#fafafa] pl-3 sm:pl-5 gap-4 sm:gap-7 p-0.5 text-xs sm:text-sm'>
              <img
                style={{ width: 15, height: 15 }}
                src='data:image/png;base64,'
                alt='wallet'
              ></img>
              Total Revenue
            </header>

            <div className='flex max-sm:gap-2 flex-col h-auto justify-center pl-3 sm:pl-5 py-2 sm:py-3'>
              <p className='text-xl sm:text-2xl md:text-3xl lg:text-4xl font-bold font-mono'>
                $ {revenue.amount}
              </p>
              <p className='text-xs sm:text-sm xl:text-sm flex  sm:flex-row gap-1 sm:gap-2 mt-1 sm:mt-3 '>
                Last updated
                <span className=''>{revenue.date}</span>
              </p>
            </div>
          </li>
          <li className=''>
            <header className='bg-[#f2f8ff] flex rounded-lg items-center pl-3 sm:pl-5 gap-4 sm:gap-7 p-0.5 text-xs sm:text-sm'>
              <img
                style={{ width: 15, height: 15 }}
                src='data:image/png;base64,'
                alt='wallet-app'
              ></img>
              Total Orders
            </header>
            <div className='flex flex-col max-sm:gap-2 h-auto justify-center pl-3 sm:pl-5 py-2 sm:py-3'>
              <p className='text-xl  sm:text-2xl md:text-3xl lg:text-4xl font-bold font-mono flex  sm:flex-row gap-5 sm:gap-2 lg:gap-4 items-baseline'>
                {totalOrders.amount}
                <span className='font-light font-sans text-base sm:text-lg md:text-xl lg:text-xl'>
                  Orders
                </span>
              </p>
              <p className='text-xs sm:text-sm flex  sm:flex-row gap-1 sm:gap-2 mt-1 sm:mt-3 '>
                Last updated
                <span className=''>1{totalOrders.date}</span>
              </p>
            </div>
          </li>
          <li className='grow'>
            <header className='bg-[#f5fdfb] flex rounded-lg items-center pl-3 sm:pl-5 gap-4 sm:gap-7 p-0.5 text-xs sm:text-sm'>
              <img
                style={{ width: 15, height: 15 }}
                src='data:image/png;base64,'
                alt='approval'
              ></img>
              Delivered Orders
            </header>
            <div className='flex flex-col h-auto justify-center pl-3 sm:pl-5 py-2 sm:py-3'>
              <p className='text-xl sm:text-2xl md:text-3xl lg:text-4xl font-semibold font-mono'>
                {deliveredOrders}
              </p>
              <p className='text-xs sm:text-sm flex gap-2 mt-1 sm:mt-3'>
                Successful orders
              </p>
            </div>
          </li>
          <li className='grow'>
            <header className='bg-[#fff5f5] flex rounded-lg items-center pl-3 sm:pl-5 gap-4 sm:gap-7 p-0.5 text-xs sm:text-sm'>
              <img
                style={{ width: 15, height: 15 }}
                src='data:image/png;base64,'
                alt='refund'
              />
              Refund
            </header>
            <div className='flex flex-col h-auto justify-center pl-3 sm:pl-5 py-2 sm:py-3'>
              <p className='text-xl sm:text-2xl md:text-3xl lg:text-4xl font-semibold font-mono'>
                ${refund.amount}
              </p>
              <p className='text-xs sm:text-sm flex gap-2 mt-1 sm:mt-3'>
                {refund.orders} order
              </p>
            </div>
          </li>
        </ul>
      </section>

      <section className='flex flex-col lg:flex-row w-full gap-3 sm:gap-5 lg:gap-10 mt-4 sm:mt-6 md:mt-8 lg:mt-10'>
        <div className='w-full lg:flex-1 h-72 sm:h-80 md:h-96 lg:h-110 shadow-xl border-gray-100 rounded-3xl p-2 sm:p-4 md:p-6 lg:p-10'>
          <AreaChart />
        </div>
        <div className='w-full lg:flex-1 h-auto lg:h-110 flex gap-3 sm:gap-5 lg:gap-10 flex-col'>
          <QuickActions />
          <Customers customers={customers} />
        </div>
      </section>
    </section>
  )
}
